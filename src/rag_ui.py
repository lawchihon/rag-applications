...
from app_functions import (convert_seconds, search_result, validate_token_threshold,
                           stream_chat, load_data, get_retriever, get_reranker,
                           get_llm)
...
## RETRIEVER
retriever = get_retriever(os.environ['WEAVIATE_ENDPOINT'], os.environ['WEAVIATE_API_KEY'], embedding_model_path)
...
def main(retriever: WeaviateWCS):
    ...
    with st.sidebar:
        ...
        alpha_input = st.slider('Select Alpha', min_value=0.0, max_value=1.0,value=0.5, step=0.01)
        retrieval_limit = st.slider('Select Retrieval Limit', min_value=1, max_value=100,value=10, step=1)
        reranker_topk = st.slider('Select Number of Top-K to Rerank', min_value=1, max_value=100, value=10, step=1)
        temperature_input = st.slider('Select Temperature', min_value=0.0, max_value=1.0, value=0.7, step=0.05)
        verbosity = st.slider('Select Verbosity', min_value=0, max_value=3, value=1, step=1)
